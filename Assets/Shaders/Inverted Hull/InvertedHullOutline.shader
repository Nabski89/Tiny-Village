	...
	SubShader
	{
		...
		Pass
		{
			...
			struct appdata
			{
				...
				#endif
				float4 color : COLOR; // This is where we receive the vertex color
...
#else
				output.position_cs = TransformObjectToHClip(input.vertex + normalize(input.normal) * _Scale * input.color.r);
#endif
